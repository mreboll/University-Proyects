...
def mostrar_ediciones():
    ...
    festivales = mongo.db.festivales.find(
        {},
...
def mostrar_festival(anyo):
    ...
    if not festival:
        abort(404) #cubre el intento de entrar a un festival no existente a través de la url

    return render_template("mostrar_actuaciones_edicion.html", anyo=festival['anyo'],
                           ciudad=festival['ciudad'], pais_organizador=festival['pais'],
...
def generar_quiz():
    ...
    anyos_db = sorted(mongo.db.festivales.distinct("anyo"), reverse=True)
    paises_db = sorted(mongo.db.festivales.distinct("concursantes.pais"))
    form = GenerarQuizForm(anyos=anyos_db, paises=paises_db)

    if form.validate_on_submit():
        return redirect(url_for('jugar_quiz',
                                anyos=form.seleccion_anyos.data,
                                paises=form.seleccion_paises.data,
...
def mostrar_actuaciones_pais(id_pais):
    ...
    # num participacion del país
    count_pipeline = [
        {"$unwind": "$concursantes"},
        {"$match": {"concursantes.id_pais": id_pais}},
        {"$count": "total"}
    ]
    num = list(mongo.db.festivales.aggregate(count_pipeline))
    total_elementos = num[0]['total'] if num else 0

    # Muestra 10 actuaciones del pais por pagina con su info
    data_pipeline = [
        {"$unwind": "$concursantes"},
        {"$match": {"concursantes.id_pais": id_pais}},
        {"$sort": {"anyo": -1}},
    ...
    participaciones = list(mongo.db.festivales.aggregate(data_pipeline))

    if not participaciones:
        abort(404) #cubre el intento de entrar a las participaciones de un país no existente a través de la url

    nombre_pais = participaciones[0]['nombre_pais']
...
def mostrar_quizzes():
    ...
    quizzes = mongo.db.quizzes.find().sort("creacion", -1).skip((pagina - 1) * elementos_por_pagina).limit(elementos_por_pagina)
...
def jugar_quiz_personalizado(nombre_quiz):
    ...
    preguntas = mongo.db.quizzes.find_one({"_id": nombre_quiz})

    if not preguntas:
        abort(404) #cubre el intento de entrar a un quiz no existente a través de la url
